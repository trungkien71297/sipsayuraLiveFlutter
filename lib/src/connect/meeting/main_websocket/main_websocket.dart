...
class MainWebSocket {
  ...
  MainWebSocket(this._meetingInfo, this._provider) {
    ...
    final uri = Uri.parse('https://abc.vuihoc.vn/graphql');
    _webSocket = GraphQLWebSocket(uri.toString(),
    ...
    try {
      ...
      switch (jsonMsgs['type']) {
        ...
        case GraphQLWSMessageType.NEXT:
          Log.debug('==kien $jsonMsgs');
          if (jsonMsgs['id'] == idGetUserCurrent) {
            someThings();
            somethings2();
          }
          break;
  ...
  void getUserCurrent() {
    idGetUserCurrent = Uuid().v4();
    _webSocket.subcribe(id: idGetUserCurrent, payload: {
      "variables": {},
      "extensions": {},
  ...
  void getUserInfo() {
    idGetUserInfo = Uuid().v4();
    _webSocket.subcribe(id: idGetUserInfo, payload: {
      "variables": {},
      "extensions": {},
